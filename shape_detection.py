...
# def extract_traffic_lights(bounding_box_list):
#     bounding_box_list.sort(key=lambda item: min(pt[0][1] for pt in item["contour"]), reverse=False)

def extract_traffic_lights(bounding_box_list, min_y_threshold=200):
    # Sort by the topmost y-coordinate in ascending order (smallest y at the top)
    bounding_box_list.sort(key=lambda item: min(pt[0][1] for pt in item["contour"]), reverse=False)

    # Filter out bounding boxes that are too high (y < 150)
    filtered_boxes = [
        box for box in bounding_box_list
        if min(pt[0][1] for pt in box["contour"]) > min_y_threshold
    ]
...
def detect_shapes(filtered_boxes):
    shape = "NULL"
    for box in filtered_boxes[:2]: 
        ...
        # fill_ratio = contour_area / bounding_box_area if bounding_box_area > 0 else 0
...
while True:
    ...
    filtered_boxes = extract_traffic_lights(bounding_box_list)
    detect_shapes(filtered_boxes)
    print(traffic_lights_list)

    # # Open the file in append mode to avoid overwriting
    # with open("moving_traffic_lights_list.txt", "a") as file:  # 'a' is append mode
    #     for shape, color in traffic_lights_list.items():
    #         file.write(f"{shape} light: {color}\n")
